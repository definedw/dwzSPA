	...
	each: function(objs, callback, args) {
		...
		if (dwz.isArray(objs)) {
			...
			for (var i = 0; i < objs.length; i++) {
				var cbResult = callback.call(objs[i], i, args);
			...
			for (var i = 0; i < objs.length; i++) {
				var elem = objs.get(i),
					flag = callback.call(elem, i, args);
	...
	if (typeof selector === "string") {
		...
		if (contextSize > 1) {
			...
			if (selector.indexOf('<') == 0) {
				this.elements = [dwz.parseHTML(selector)]; // html字符串转dom
			} else {
...
dwz.fn = dwz.extend(dwz.prototype.init.prototype, {
	...
	parentsUnitBox: function() {
		return this.parentsUntil(function() {
			return dwz.hasClass(this, dwz.config.unitBox);
		});
	},
	...
	event: {
		...
		isBind: function(elem, type, fn){
			...
			if (_events[type] && _events[type].length > 0) {
				if (fn !== undefined){
					for (var i=0; i<_events[type].length; i++) {
						if (fn === _events[type][i].fn) { return true; }
					}
				} else {
					return true;
				}
...
dwz.fn.extend({
	...
	/**
	 * $box.on('touchstart', function(event, data){
	...
	on: function(types, fn, data) {
		return this.each(function() {
			dwz.event.add(this, types, fn, data);
		});
	},
	...
	click: function(fn, data) {
		return this.on($.event.hasTouch ? 'touchstart' : 'click', fn, data);
	},
	change: function(fn, data) {
		return this.on('change', fn, data);
	},
	offset: function() {
	...
	prepend: function(content) {
		...
		this.get(0).insertBefore(elem, this.get(0).firstChild);
		return this;
	},
	append: function(content) {
		var elem = $(content).get(0);
		this.get(0).appendChild(elem);
		return this;
	},
	...
	valHooks: {
		...
		select: {
			...
			set: function(elem, value) {
				...
				while (i--) {
					option = options[i];
					if ((option.selected = dwz.inArray(option.value, values) >= 0)) {
						optionSet = true;
					}
				}
	...
	dwz.valHooks[this] = {
		...
		set: function(elem, value) {
			if (dwz.isArray(value)) {
				return (elem.checked = dwz.inArray(dwz(elem).val(), value) >= 0);
			}
		}
	};
});
...
dwz.extend({
	...
	ajaxSetup: function(settings) {
		return dwz.extend(jQuery.ajaxSettings, settings);
	},
	/**
	 *
	 *
