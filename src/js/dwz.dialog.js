...
$.dialog = {
    config: {
        ...
        openClass: 'open',
        frag: '<div id="dialog" class="pop-window unitBox"><div class="pop-header"><span class="title">弹出框</span><a class="pop-close" href="javascript:$.dialog.close()"><i class="icon icon-close"></i></a></div><div class="pop-content"></div></div>',
    ...
    init: function(options) {
        ...
        $('body').append($.dialog.config.frag);
        $('body').append($.dialog.config.bgFrag);
    ...
    loadExternal: function(url) {
        var $box = this.$box.html('<a class="video-close" href="javascript:$.dialog.close()"><i class="icon icon-close-dialog"></i></a><div class="pop-content"></div>');
    ...
    _closeDirect: function() {
        ...
        $box.animate({y:document.documentElement.clientHeight}, 500, 'ease', function(){
            $box.hide().html('').removeClass($.dialog.config.fullscreenClass);
        });
